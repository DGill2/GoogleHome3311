...
app.intent(GENDER_INTENT, (conv, params, genders)=>{
  ...
  if(conv.data.sex === 'male' || conv.data.sex === 'man'|| conv.data.sex === 'guy' || conv.data.sex === 'father')
  {
    console.log(" in male if statement");
    conv.user.storage.sex = 'male';  
    conv.ask('Great!. what is your age?');
  }
  else if(conv.data.sex === 'female' || conv.data.sex === 'girl'|| conv.data.sex === 'mother')
  {
...
app.intent(EMAIL_INTENT, (conv, params, emailOption)=> {
    ...
    conv.close('<speak> Sorry. At the moment, this feature is being implemented <break time="0.7" />. I apologize for the inconvenience. <break time="1" /> Now. I will self-destruct in 3, <break time="1" />' +
...
/*calculate the BMI, 
tell the user BMI is rough example of their health measurements
tell them the works out
ask them if they would like me to email them all the infomation to the google account thats linked to this google home
*/
  //BMI needs weight in kg and height in meters for calculation
// function arr()
// {
//     var workOuts = Math.random()*["pushups", "situps", "cardio", "curls"];
// }
